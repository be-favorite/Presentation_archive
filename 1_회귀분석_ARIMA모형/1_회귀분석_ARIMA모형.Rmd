...
\begin{align} \hat{y} = f(x_1, x_2, \cdots, x_p). \end{align}
...
\begin{align} y = \beta_0 + \beta_1 x + \epsilon, \epsilon \sim N(0, \sigma^2). \end{align}
...
<!-- 식 (2)의 의미는 예측변수와 반응변수 간 선형관계가 성립한다고 전제될 때, 주어진 $x$에서 미지의 선형관계($\beta_0 + \beta_1 x$)로부터 오차($\epsilon)$가 더해져 관측값 $y$가 주어짐을 뜻함. -->
...
\begin{align} \hat{y} = \hat{\beta}_0 + \hat{\beta}_1 x \end{align}
...
```{r simple, warning = FALSE, fig.height=4.5, fig.width=8, fig.retina = 2, fig.align = "center"}
...
```{r residual, warning = FALSE, fig.height=3.5, fig.width=8, fig.retina = 2, fig.align = "center"}
par(mfrow = c(1, 2))
plot(lm_mod, 1)
plot(lm_mod, 2)
```
...
\begin{align} y = \alpha + \beta_{1} x_1 + \beta_2 x_2 + \cdots + \beta_p x_p + \epsilon, \epsilon \sim N(0, \sigma^2) \end{align}
...
\begin{align} \hat{y} = \hat{\beta}_0 + \hat{\beta}_1 x_1 + \hat{\beta}_2 x_2 \end{align}

---

# 다중회귀모형

\begin{align} \boldsymbol{\beta} = (X^TX)^{-1}X^Ty \end{align}
...
\begin{align} {\rm{VIF}}_j = {{1}\over{1-R_j^2}} \end{align}
